...
function updateCurrentWeather(data, cityStateName) {
  ...
  if(apiKey != '') {
    ...
    document.getElementById('current-city-state').appendChild(addFavorite);
  }
...
document.getElementById('submit-login').addEventListener('click', (e) => {
  ...
  $.post(`https://sweater-weather-1810.herokuapp.com/api/v1/sessions?email=j${username}&password=${password}`, (response) => {
    if(response.api_key) {  
      let apiKey = response.api_key;
      document.cookie = `apiKey=${apiKey}`;
      document.cookie = `username=${username}`;
